...
export function useMarkdownContent(locale: 'en' | 'ru' | 'fr' | 'es' | 'ar'): MarkdownContent {
  ...
  return {
    ...
    getPostBySlug: (slug: string) => {
      const post = syncData.posts?.find((p: any) => p.slug === slug);
      if (post) return post;
      // Fallback to English if not found in current locale
      if (locale !== 'en') {
        const enData = markdownContent['en'] || { posts: [] };
        const enPost = enData.posts?.find((p: any) => p.slug === slug);
        if (enPost) return { ...enPost, isFallback: true };
      }
      return null;
    },
    getProductBySlug: (slug: string) => {
      const product = syncData.products?.find((p: any) => p.slug === slug);
      if (product) return product;
      // Fallback to English if not found in current locale
      if (locale !== 'en') {
        const enData = markdownContent['en'] || { products: [] };
        const enProduct = enData.products?.find((p: any) => p.slug === slug);
        if (enProduct) return { ...enProduct, isFallback: true };
      }
      return null;
    },
    isLoading,
