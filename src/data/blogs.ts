export interface BlogPost {
  id: string;
  slug: string;
  title: string;
  excerpt: string;
  content: string;
  date: string;
  author: string;
  category: string;
  tags: string[];
  image: string;
}

const HERO_IMG = "https://mgx-backend-cdn.metadl.com/generate/images/1044526/2026-03-20/0821521e-e54b-4dff-b118-4f4b9ba70803.png";
const LAB_IMG = "https://mgx-backend-cdn.metadl.com/generate/images/1044526/2026-03-20/08108beb-b4ff-4efc-b6fa-1c545073bedd.png";
const SHIP_IMG = "https://mgx-backend-cdn.metadl.com/generate/images/1044526/2026-03-20/e6ad8468-80af-478a-a520-d0ca3ba12cdb.png";
const PRODUCT_IMG = "https://mgx-backend-cdn.metadl.com/generate/images/1044526/2026-03-20/eb1c2135-5209-41f4-9875-561825e0cc52.png";

export const blogPosts: BlogPost[] = [
  {
    id: "10",
    slug: "comprehensive-guide-to-oxalic-acid-properties-applications-and-safety",
    title: "Comprehensive Guide to Oxalic Acid: Properties, Applications, and Safety",
    excerpt: "Deep dive into Oxalic Acid (ethanedioic acid). Explore its chemical properties, industrial applications in rare earth extraction and metal treatment, and essential safety guidelines for 2026.",
    content: `## What is Oxalic Acid? (SGE Quick Answer)
**Oxalic Acid**, chemically known as **ethanedioic acid (H₂C₂O₄)**, is a powerful organic acid characterized by its strong acidity, reducing properties, and ability to form stable complexes with metal ions. It is essential in industries such as **rare earth extraction**, **metal cleaning**, and **pharmaceutical manufacturing**. As a leading [high-purity Oxalic Acid supplier](https://www.sinopeakchem.com/en/products/oxalic-acid), Sinopeakchem provides technical-grade solutions for global industrial needs.

---

## Introduction: Our Perspective on Oxalic Acid
In our years of experience supplying chemicals globally, we've seen how Oxalic Acid, though seemingly simple, acts as a cornerstone for advanced industrial processes. Whether it's restoring the luster to stainless steel or separating critical rare earth elements, its chemical versatility is unmatched. This guide provides a deep analysis of its properties, production, and the rigorous safety standards we uphold at Sinopeakchem.

## Chapter 1: Physicochemical Properties and Diverse Applications

Oxalic Acid exists as a colorless, transparent crystalline solid, highly soluble in water. Its unique ability to \"chelate\" or bind with metal ions makes it indispensable.

### 1. Textile Dyeing and Printing Industry
*   **Bleaching and Purification**: It effectively removes pigments and iron stains from fabrics.
*   **pH Regulation**: Essential for creating stable acidic environments for specific dyes.
*   **Water Softening**: Complexes with calcium and magnesium to prevent dye precipitation.

### 2. Metal Treatment and Rust Removal
Oxalic acid is an ideal rust remover. It reacts with iron oxide to form soluble complexes, making it a favorite for maintaining infrastructure and heavy machinery. It is also increasingly used in **semiconductor manufacturing** for wafer cleaning.

### 3. Rare Earth Separation and Extraction
This is perhaps its most critical role. By adjusting pH levels, Oxalic Acid achieves highly selective precipitation of rare earth elements, separating them from impurities like iron and aluminum. Our technical team often assists clients in optimizing these precipitation parameters to improve recovery rates.

### 4. Pharmaceutical Intermediates
Oxalic acid is a key intermediate in producing drugs like tetracycline and phenobarbital. The pharmaceutical industry requires extreme purity, which is why we offer specialized [pharmaceutical-grade Oxalic Acid](https://www.sinopeakchem.com/en/products/oxalic-acid).

---

## Chapter 2: Production Processes and Safety Management

### Mainstream Production Methods
1.  **Carbonyl Synthesis**: A clean, modern process reacting carbon monoxide with alcohols.
2.  **Sodium Formate Method**: A traditional, mature process but with higher energy consumption.
3.  **Oxidation Method**: Uses carbohydrates like glucose as raw materials.

### Safety and Toxicological Risks
Oxalic Acid is toxic and corrosive. **Based on our safety protocols, we emphasize:**
*   **Acute Toxicity**: Ingestion of even small amounts (2-5g) can be fatal.
*   **Corrosivity**: Direct skin contact causes severe burns; prolonged exposure can lead to cyanosis or gangrene.
*   **Respiratory Hazards**: Inhalation of dust irritates the mucosa and can cause respiratory burns.

### Personal Protection Equipment (PPE)
When handling Oxalic Acid, our team always recommends:
*   Professional protective gloves and safety goggles.
*   Respirators for dust-prone environments.
*   Acid-resistant clothing.

---

## Chapter 3: Market Trends and Future Outlook

The global Oxalic Acid market is projected to reach **$1.04 billion by 2034**, driven by:
*   **New Energy**: Demand for battery material purification (Lithium-ion batteries).
*   **High-Tech Electronics**: Ultra-low impurity grades for semiconductor manufacturing.
*   **Environmental Sustainability**: Its use as an eco-efficient cleaning agent compared to harsher alternatives.

---

## FAQ: Common Questions About Oxalic Acid

### What is the primary use of Oxalic Acid in industry?
The primary uses include rust removal, textile bleaching, and as a precipitating agent in rare earth element extraction.

### Is Oxalic Acid safe for household use?
While it is found in some cleaning products, it is highly toxic and corrosive. It must be handled with extreme care, using gloves and proper ventilation.

### How should Oxalic Acid be stored?
It should be stored in a cool, dry, well-ventilated area, away from moisture, oxidizing agents, and alkaline substances.

## Conclusion
As a trusted [Oxalic Acid supplier from China](https://www.sinopeakchem.com/en/products/oxalic-acid), Sinopeakchem is committed to delivering high-purity products (≥99.6%) alongside comprehensive technical support. For bulk inquiries or specialized grade requirements, feel free to contact our expert team.`,
    date: "2026-04-15",
    author: "Sinopeakchem Technical Team",
    category: "Product Guide",
    tags: ["oxalic acid", "industrial chemicals", "rare earth extraction", "safety management"],
    image: "/images/blog/oxalic-acid-header.webp",
  },
  {
    id: "7",
    slug: "sodium-thiosulphate-versatile-chemical-for-diverse-industrial-applications",
    title: "Sodium Thiosulphate: The Versatile Chemical for Diverse Industrial Applications",
    excerpt: "An in-depth look at the diverse applications of Sodium Thiosulphate, from water treatment and photography to mining and medicine. Learn why it is a crucial chemical for various industrial processes.",
    content: `## Introduction to Sodium Thiosulphate\n\nSodium Thiosulphate (Na₂S₂O₃), often referred to as sodium hyposulfite or simply \"hypo,\" is a fascinating inorganic salt with a rich history and an even richer array of modern industrial applications. Known for its reducing properties and ability to complex with metal ions, this white, crystalline compound plays a critical role in sectors ranging from water treatment to photography, and from mining to medicine. Its versatility makes it an indispensable chemical for businesses seeking efficient and reliable solutions across various processes.\n\n## Key Applications of Sodium Thiosulphate\n\n### 1. Water Treatment and Dechlorination\n\nOne of the most widespread and vital applications of Sodium Thiosulphate is in water treatment, particularly for dechlorination. Chlorine and chloramines are commonly used disinfectants in municipal water supplies and industrial wastewater. However, their residual presence can be harmful to aquatic life, interfere with industrial processes, or pose health risks. Sodium Thiosulphate rapidly and effectively neutralizes these chlorine compounds, converting them into harmless chlorides and sulfates. This makes it an ideal choice for:\n\n*   **Aquaculture**: Protecting fish and other aquatic organisms from chlorine toxicity.\n*   **Breweries and Distilleries**: Removing chlorine that can impart off-flavors to beverages.\n*   **Dialysis Centers**: Ensuring water used in medical procedures is free from chlorine.\n*   **Industrial Cooling Towers**: Preventing corrosion and biofouling while managing discharge water quality.\n\n### 2. Photographic Fixing Agent\n\nHistorically, Sodium Thiosulphate gained prominence as the primary fixing agent in traditional silver-halide photography. After exposure and development, unexposed silver halide crystals remain on the film or photographic paper. Sodium Thiosulphate dissolves these unexposed crystals, making the image permanent and light-insensitive. This process, known as \"fixing,\" is crucial for the longevity and stability of photographic prints and negatives. While digital photography has largely replaced traditional methods, Sodium Thiosulphate remains essential for niche photographic processes and archival purposes.\n\n### 3. Gold and Silver Extraction (Thiosulphate Leaching)\n\nIn the mining industry, Sodium Thiosulphate is emerging as an environmentally friendlier alternative to cyanide for the leaching of gold and silver from ores. Cyanide leaching, while effective, carries significant environmental risks. Thiosulphate leaching offers a less toxic pathway for precious metal recovery, particularly from refractory (difficult-to-process) ores. This method is gaining traction as mining companies seek more sustainable and responsible extraction techniques, aligning with global environmental regulations and corporate social responsibility initiatives.\n\n### 4. Textile and Leather Industries\n\nSodium Thiosulphate finds several applications in the textile and leather sectors:\n\n*   **Antichlor**: After bleaching fabrics or leather with chlorine-based agents, residual chlorine must be removed to prevent damage to the fibers and dyes. Sodium Thiosulphate acts as an \"antichlor,\" neutralizing any remaining chlorine.\n*   **Dyeing**: It can be used as a reducing agent in certain dyeing processes and as a mordant to help dyes bind more effectively to fabrics.\n*   **Chrome Tanning**: In leather processing, it can be used to reduce chromate, a key component in chrome tanning, making the process more efficient and environmentally sound.\n\n### 5. Medical and Pharmaceutical Uses\n\nBeyond industrial applications, Sodium Thiosulphate has important medical uses. It is recognized as an antidote for cyanide poisoning, working by converting cyanide into less toxic thiocyanate. It is also used in the treatment of calciphylaxis and as an antifungal agent in some dermatological preparations. Its role in medicine underscores its diverse chemical properties and therapeutic potential.\n\n## Why Choose a Reliable Sodium Thiosulphate Supplier from China?\n\nWhen sourcing Sodium Thiosulphate for your industrial needs, partnering with a reputable Chinese supplier like Sinopeakchem offers significant advantages. China is a leading global producer of chemicals, benefiting from economies of scale, advanced supply capabilities, and efficient logistics networks. A reliable supplier ensures:\n\n*   **Consistent Quality**: Adherence to international standards (e.g., industrial grade, photographic grade) and rigorous quality control processes.\n*   **Competitive Pricing**: Cost-effective sourcing without compromising on product integrity.\n*   **Stable Supply Chain**: Robust supply capacity and efficient export procedures from major ports like Qingdao, Tianjin, and Dalian.\n*   **Comprehensive Documentation**: Provision of essential documents such as Certificate of Analysis (COA) and Material Safety Data Sheet (MSDS) for compliance and safety.\n\n## Conclusion\n\nSodium Thiosulphate is a chemical powerhouse, offering solutions across a multitude of industries. Its efficacy in dechlorination, photographic fixing, precious metal recovery, and textile processing, coupled with its medical applications, highlights its broad utility. For businesses looking to leverage the benefits of this versatile compound, securing a high-quality supply from an experienced and reliable Chinese chemical supplier is paramount. By understanding its diverse applications and choosing the right partner, you can ensure operational efficiency, product quality, and environmental responsibility. Explore Sinopeakchem\'s range of high-purity Sodium Thiosulphate and discover how we can support your industrial requirements.`,
    date: "2026-04-15",
    author: "Sinopeakchem",
    category: "Chemical Insights",
    tags: ["Sodium Thiosulphate", "Dechlorination", "Water Treatment", "Chemical Supplier"],
    image: PRODUCT_IMG,
  },
  {
    id: "6",
    slug: "how-to-choose-the-right-chem-supplier-from-china",
    title: "How to Choose the Right Chemical Supplier from China: A Comprehensive Guide",
    excerpt: "An in-depth guide on selecting a reliable chemical supplier in China, covering supplier qualification, quality control, supply chain, cost-effectiveness, and communication.",
    content: `## Introduction\n\nIn today's globalized world, China plays a pivotal role as a world factory in the chemical supply sector. For buyers seeking **high-quality, cost-effective** chemicals, choosing a **reliable Chinese chemical supplier** is crucial. However, with numerous options available, making an informed decision can be daunting. This article provides a comprehensive guide to help you find the most suitable partner in a complex market.\n\n## 1. Assess Supplier Qualifications and Reputation\n\nThe first step in selecting a **Chinese chemical supplier** is to thoroughly assess their qualifications and market reputation. This includes:\n\n*   **Business Licenses and Certifications**: Ensure the supplier has a valid business license and relevant industry certifications, such as ISO 9001 (Quality Management System) and ISO 14001 (Environmental Management System). These certifications are basic proof of their compliance with international standards.\n*   **Industry Experience and Expertise**: Examine the supplier's experience in your required chemical field. An experienced supplier usually has a deeper understanding of product characteristics, market trends, and potential risks.\n*   **Customer Reviews and Case Studies**: Read reviews and success stories from other customers. Real customer feedback provides valuable insights into the supplier's service quality and reliability.\n*   **Transparency**: A trustworthy supplier should maintain a high degree of transparency regarding their supply processes, quality control measures, and supply chain links.\n\n## 2. Strict Quality Control System\n\nIn **chemical procurement**, product quality is non-negotiable. Therefore, the supplier's **quality control** system is a key evaluation factor:\n\n*   **Quality Management Process**: Understand how the supplier conducts raw material inspection, supply process control, and final product testing. Do they have their own laboratory? Do they provide detailed Certificates of Analysis (COA) and Material Safety Data Sheets (MSDS)?\n*   **Sample Testing**: Before making a bulk purchase, always request product samples for independent testing. This ensures the product meets your technical specifications and quality standards.\n*   **Traceability**: Ask if the supplier can provide batch information and supply records for traceability in case of issues.\n\n## 3. Sound Supply Chain and Logistics Capabilities\n\nEfficient **supply chain management** is essential for ensuring timely and safe delivery of chemicals:\n\n*   **supply Capacity and Inventory**: Evaluate the supplier's supply scale and inventory levels to ensure they can meet your order volume and delivery time requirements.\n*   **Packaging and Storage**: Chemicals often require special packaging and storage conditions. Confirm that the supplier complies with international shipping standards and can provide appropriate packaging solutions.\n*   **Logistics Network**: Understand the supplier's logistics partners and shipping routes. Can they offer flexible shipping options (e.g., sea, air, land) and handle hazardous materials transport?\n*   **Export Experience**: Choose a supplier with extensive experience in international trade and exports. They can better handle the complexities of customs, clearance, and international shipping.\n\n## 4. Cost-Effectiveness and Pricing Strategy\n\nWhile price is an important consideration, **cost-effectiveness** is much more than that. You need to consider:\n\n*   **Total Cost**: In addition to the unit price, consider transportation costs, insurance fees, tariffs, inspection fees, and potential quality issue costs. A seemingly low-priced product could end up costing more if it leads to high logistics or quality problems.\n*   **Long-Term Partnership**: Building a long-term, stable relationship with a supplier can often lead to more favorable prices and better service.\n*   **Payment Terms**: Negotiate reasonable payment terms to balance cash flow and risk.\n\n## 5. Communication and Customer Service\n\nEffective communication is the cornerstone of a successful partnership:\n\n*   **Response Speed**: The supplier's response speed reflects their service level. Choose suppliers who can promptly answer your inquiries and resolve issues.\n*   **Language Skills**: Ensure the supplier's sales and technical teams have good English communication skills to avoid misunderstandings.\n*   **Technical Support**: A good supplier should be able to provide necessary technical support and product application advice.\n\n## 6. Compliance and Sustainability\n\nUnder increasingly stringent global environmental regulations, **compliance** and sustainability are becoming more important:\n\n*   **Environmental Standards**: Find out if the supplier complies with China's environmental regulations and adopts sustainable supply practices.\n*   **Social Responsibility**: Investigate whether the supplier is concerned about employee welfare and social responsibility.\n\n## Conclusion\n\nChoosing the right **Chinese chemical supplier** is a complex process involving multiple considerations. By comprehensively evaluating qualifications, quality control, supply chain capabilities, cost-effectiveness, communication, and compliance, you will be able to find a trustworthy partner to ensure your chemical procurement process is smooth and efficient. Remember, building a long-term, mutually beneficial partnership will be key to your business success.`,
    date: "2026-04-15",
    author: "Sinopeakchem",
    category: "Sourcing Guide",
    tags: ["chemical supplier", "sourcing", "China", "procurement", "supply chain"],
    image: SHIP_IMG,
  },
  {
    id: "1",
    slug: "complete-guide-sodium-thiosulphate-applications",
    title: "Complete Guide to Sodium Thiosulphate: Applications, Properties, and Industrial Uses",
    excerpt: "Discover the versatile applications of sodium thiosulphate across water treatment, photography, mining, and more. Learn about its chemical properties and how to choose the right grade for your needs.",
    content: `## Introduction
Sodium thiosulphate (Na2S2O3) is a versatile inorganic compound with numerous industrial applications. This guide explores its properties and how it's used across various sectors.
## Key Applications
### 1. Water Treatment
Used for dechlorination in municipal water treatment and aquaculture. It effectively neutralizes residual chlorine.
### 2. Photography
Known as "hypo," it serves as a fixing agent to dissolve unexposed silver halides on film and paper.
### 3. Mining
Used in gold and silver extraction as an alternative to cyanide leaching.
### 4. Medical
Used as an antidote for cyanide poisoning and in various dermatological treatments.
## Conclusion
Choosing the right grade of sodium thiosulphate is essential for optimal performance in your specific application. Sinopeakchem offers high-purity sodium thiosulphate for all industrial needs.`,
    date: "2026-04-15",
    author: "Sinopeakchem Technical Team",
    category: "Product Guide",
    tags: ["sodium thiosulphate", "water treatment", "chemical guide"],
    image: PRODUCT_IMG,
  },
  {
    id: "2",
    slug: "how-to-choose-right-caustic-soda-grade",
    title: "How to Choose the Right Caustic Soda Grade for Your Industry",
    excerpt: "A comprehensive comparison of caustic soda grades, forms, and applications to help you make the best purchasing decision for your specific industrial requirements.",
    content: `## Understanding Caustic Soda Grades
Caustic Soda (Sodium Hydroxide) is available in several grades and forms. Choosing the right one is crucial for your process efficiency and cost-effectiveness.
## Common Forms
### 1. Caustic Soda Flakes
- Purity: Typically 99%
- Easy to store and handle
- Ideal for small to medium scale applications
### 2. Caustic Soda Pearls
- Purity: Typically 99%
- Dust-free and free-flowing
- Excellent for precise dosing
### 3. Caustic Soda Lye (Liquid)
- Concentration: Usually 32% or 50%
- Ready to use in large scale industrial processes
- Lower handling costs for bulk users
## Industrial Applications
- **Textile Industry**: For scouring and mercerization
- **Soap & Detergent**: Key ingredient in saponification
- **Water Treatment**: For pH adjustment and regeneration of ion exchange resins
- **Paper Industry**: In the pulping and bleaching process
## Conclusion
Whether you need flakes, pearls, or liquid caustic soda, Sinopeakchem provides high-quality products that meet international standards.`,
    date: "2026-04-15",
    author: "Sinopeakchem Technical Team",
    category: "Product Guide",
    tags: ["caustic soda", "sodium hydroxide", "industrial grades"],
    image: LAB_IMG,
  },
  {
    id: "3",
    slug: "chemical-shipping-logistics-china-guide",
    title: "Chemical Shipping and Logistics from China: What You Need to Know",
    excerpt: "Navigate the complexities of international chemical shipping from China. Learn about regulations, packaging requirements, and choosing the right logistics partner.",
    content: `## Navigating Chemical Logistics
Shipping chemicals internationally requires specialized knowledge and careful planning. Here's what you need to consider when importing from China.
## Key Considerations
### 1. Regulations and Compliance
- Understand IMDG codes for hazardous materials
- Ensure all MSDS/SDS documentation is up to date
- Comply with both Chinese export and destination country import laws
### 2. Packaging Requirements
- Use UN-approved packaging for dangerous goods
- Ensure proper labeling and marking
- Consider temperature-controlled shipping if necessary
### 3. Choosing a Logistics Partner
- Look for experience in chemical handling
- Verify their network of specialized carriers
- Check their track record for on-time delivery and safety
## How Sinopeakchem Helps
We manage the entire logistics process, from factory to port, ensuring all regulations are met and your shipment arrives safely. Sinopeakchem provides comprehensive shipping support to ensure smooth delivery worldwide.`,
    date: "2026-04-15",
    author: "Sinopeakchem Logistics Team",
    category: "Shipping & Logistics",
    tags: ["chemical shipping", "international trade", "logistics"],
    image: SHIP_IMG,
  },
  {
    id: "4",
    slug: "water-treatment-chemicals-comparison",
    title: "Water Treatment Chemicals Comparison: PAC vs Aluminum Sulfate vs Ferric Chloride",
    excerpt: "Compare the most popular water treatment coagulants including Polyaluminum Chloride (PAC), Aluminum Sulfate, and Ferric Chloride to find the best solution for your water treatment needs.",
    content: `## Introduction
Choosing the right coagulant is crucial for effective water treatment. This article compares three popular options: PAC, Aluminum Sulfate, and Ferric Chloride.
## Polyaluminum Chloride (PAC)
### Advantages
- Effective over a wide pH range (5.0-9.0)
- Lower dosage required compared to alum
- Faster floc formation
- Less sludge supply
- Lower residual aluminum in treated water
### Best For
- Municipal drinking water treatment
- Industrial wastewater
- High-turbidity water sources
## Aluminum Sulfate (Alum)
### Advantages
- Lower cost per unit
- Well-established technology
- Easy to handle and dose
- Effective for most water types
### Limitations
- Narrower effective pH range (5.5-7.5)
- Higher dosage required
- More sludge supply
- Higher residual aluminum
## Ferric Chloride
### Advantages
- Effective for heavy metal removal
- Works well in cold water
- Good for phosphorus removal
- Effective at low pH
### Limitations
- Can cause color issues
- Corrosive to equipment
- More expensive than alum
## Comparison Table
| Parameter | PAC | Alum | Ferric Chloride |
|-----------|-----|------|-----------------|
| Effective pH | 5.0-9.0 | 5.5-7.5 | 4.0-11.0 |
| Dosage | Low | Medium | Medium |
| Floc Speed | Fast | Medium | Fast |
| Sludge Volume | Low | High | Medium |
| Cost | Medium | Low | High |
## Conclusion
PAC offers the best overall performance for most water treatment applications. Contact Sinopeakchem for high-quality PAC and other water treatment chemicals.`,
    date: "2026-04-15",
    author: "Sinopeakchem Technical Team",
    category: "Technical Article",
    tags: ["water treatment", "PAC", "coagulant", "chemical comparison"],
    image: LAB_IMG,
  },
  {
    id: "5",
    slug: "oxalic-acid-industrial-applications",
    title: "Oxalic Acid: Industrial Applications and Safety Guidelines",
    excerpt: "Explore the diverse industrial applications of oxalic acid from rust removal to rare earth extraction, along with important safety and handling guidelines.",
    content: `## What is Oxalic Acid?
Oxalic Acid (C₂H₂O₄) is the simplest dicarboxylic acid and one of the strongest organic acids. It occurs naturally in many plants and is supplied industrially for numerous applications.
## Key Industrial Applications
### 1. Metal Cleaning and Rust Removal
Oxalic acid is highly effective at dissolving iron oxide (rust) and is widely used for:
- Cleaning metal surfaces before coating
- Removing rust stains from concrete and stone
- Brightening aluminum and stainless steel
### 2. Marble and Stone Polishing
In the stone industry, oxalic acid is used to:
- Polish marble and granite surfaces
- Remove stains from natural stone
- Restore shine to weathered stone
### 3. Textile and Leather Industry
Applications include:
- Bleaching cotton and linen fabrics
- Removing iron stains from textiles
- Leather processing and tanning
### 4. Rare Earth Element Extraction
Oxalic acid plays a crucial role in:
- Precipitating rare earth oxalates from solution
- Purifying rare earth concentrates
- Producing high-purity rare earth oxides
### 5. Pharmaceutical Industry
Used as an intermediate in:
- Antibiotic synthesis
- Vitamin C supply
- Various pharmaceutical compounds
## Safety Guidelines
### Handling Precautions
- Wear appropriate PPE (gloves, goggles, respirator)
- Work in well-ventilated areas
- Avoid contact with skin and eyes
- Do not ingest
### Storage Requirements
- Store in cool, dry, well-ventilated area
- Keep away from heat sources and incompatible materials
- Use tightly sealed containers
- Keep away from strong oxidizers and bases
## Conclusion
Oxalic acid is an essential industrial chemical with diverse applications. Sinopeakchem supplies high-purity oxalic acid (≥99.6%) with complete safety documentation. Contact us for pricing and availability.`,
    date: "2026-04-15",
    author: "Sinopeakchem Technical Team",
    category: "Product Guide",
    tags: ["oxalic acid", "rust removal", "industrial chemicals", "safety"],
    image: PRODUCT_IMG,
  },
];

export function getBlogBySlug(slug: string): BlogPost | undefined {
  return blogPosts.find((b) => b.slug === slug);
}

export function getRecentBlogs(limit = 3): BlogPost[] {
  return [...blogPosts].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, limit);
}
