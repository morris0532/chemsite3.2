...
export default function BlogPage() {
  ...
  const currentLocale = isRu ? 'ru' : (isFr ? 'fr' : (isEs ? 'es' : (isAr ? 'ar' : 'en')));
  const { posts: markdownPosts, isLoading } = useMarkdownContent(currentLocale);
  const { posts: enPosts } = useMarkdownContent('en');

  const allPosts = useMemo(() => {
    if (currentLocale === 'en') return markdownPosts;
    
    // Start with English posts as base
    const basePosts = [...enPosts];
    
    // Replace with translated versions where available
    return basePosts.map(enP => {
      const translated = markdownPosts.find((p: any) => 
  ...
  }, [currentLocale, markdownPosts, enPosts]);
              ...
              {gridPosts.map((post) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <div className={`flex items-center justify-between ${isAr ? 'flex-row-reverse' : ''}`}>
                      <div className={`flex items-center gap-1.5 text-[#0066B3] font-bold group/btn ${isAr ? 'flex-row-reverse' : ''}`}>
                        <span className={`text-sm ${isAr ? 'font-arabic' : ''}`}>{content.readMore}</span>
                        {isAr ? <ArrowLeft className="w-4 h-4 transition-transform group-hover/btn:-translate-x-1.5" /> : <ArrowRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1.5" />}
                      </div>
                      {post.isFallback && (
                        <span className="flex items-center gap-1 text-[9px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-full border border-amber-100">
                          EN
                ...
                <div className="flex items-center gap-1 mx-2">
                  {Array.from({ length: totalPages }).map((_, i) => (
                    <Button
                      ...
                      className={`w-10 h-10 rounded-xl font-bold ${currentPage === i + 1 ? 'bg-[#0066B3]' : 'text-slate-500'}`}
                    >
                      {i + 1}
                    </Button>
                  ))}
