...
import { 
  ...
  Globe, Shield, Zap, Award, Microscope, MessageCircle, Home, BookOpen
} from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  const [docSubscribe, setDocSubscribe] = useState(true);
  ...
  const handleInquirySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Inquiry Sent",
          description: "We have received your request and sent a confirmation email.",
        });
        setTimeout(() => {
  ...
  const content = isRu ? {
    ...
    subscribeText: "Подпишитесь на нашу рассылку, чтобы получать обновления о продуктах и новости отрасли.",
    exploreRelatedTopic: "Изучить связанную тему",
    topicDescription: "Погрузитесь глубже в области применения, аналитику рынка и технические статьи, связанные с этим продуктом.",
    viewTopicHub: "Перейти в раздел тем"
  } : (isFr ? {
    ...
    subscribeText: "Abonnez-vous à notre newsletter pour recevoir les mises à jour sur les produits et les actualités du secteur.",
    exploreRelatedTopic: "Explorer le Sujet Connexe",
    topicDescription: "Plongez plus profondément dans les applications, les analyses de marché et les articles techniques liés à ce produit.",
    viewTopicHub: "Voir le Centre de Sujets"
  } : (isEs ? {
    ...
    subscribeText: "Suscríbase a nuestro boletín para recibir actualizaciones de productos e información del sector.",
    exploreRelatedTopic: "Explorar Tema Relacionado",
    topicDescription: "Profundice en las aplicaciones, los conocimientos del mercado y los artículos técnicos relacionados con este producto.",
    viewTopicHub: "Ver Centro de Temas"
  } : (isAr ? {
    ...
    subscribeText: "اشترك في نشرتنا الإخبارية للحصول على تحديثات المنتجات ورؤى الصناعة.",
    exploreRelatedTopic: "استكشاف الموضوعات ذات الصلة",
    topicDescription: "تعمق أكثر في التطبيقات ورؤى السوق والمقالات الفنية المتعلقة بهذا المنتج.",
    viewTopicHub: "عرض مركز الموضوعات"
  } : {
    ...
    subscribeText: "Subscribe to our newsletter, for product updates and industry insights.",
    exploreRelatedTopic: "Explore Related Topic",
    topicDescription: "Dive deeper into the applications, market insights, and technical articles related to this product.",
    viewTopicHub: "View Topic Hub"
  })));
              ...
              <div className="lg:pl-12">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  {product.status === 'inactive' && (
                    <>
                      <span className="text-slate-300">/</span>
                      <span className="px-4 py-2 bg-amber-100 text-amber-800 text-xs font-black uppercase tracking-[0.15em] rounded-xl border-2 border-amber-200 flex items-center gap-2 shadow-sm animate-pulse">
                        <Info className="w-4 h-4 text-amber-600" />
                        {isRu ? "Временно недоступно" : (isFr ? "Temporairement indisponible" : (isEs ? "Temporalmente no disponible" : (isAr ? "غير متوفر مؤقتاً" : "Temporarily Unavailable")))}
                      </span>
                    </>
                  )}
                </div>
                      ...
                      {inquirySubmitted ? (
                        ...
                        <form onSubmit={handleInquirySubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          <div className="space-y-2 md:col-span-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Полное имя *" : (isFr ? "Nom complet *" : (isEs ? "Nombre completo *" : (isAr ? "الاسم الكامل *" : "Full Name *")))}</Label>
                            <Input required placeholder="John Doe" className="h-12 rounded-xl border-slate-200" value={inquiryName} onChange={e => setInquiryName(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Электронная почта *" : (isFr ? "Adresse e-mail *" : (isEs ? "Correo electrónico *" : (isAr ? "البريد الإلكتروني *" : "Email Address *")))}</Label>
                            <Input required type="email" placeholder="john@company.com" className="h-12 rounded-xl border-slate-200" value={inquiryEmail} onChange={e => setInquiryEmail(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Номер телефона *" : (isFr ? "Numéro de téléphone *" : (isEs ? "Número de teléfono *" : (isAr ? "رقم الهاتف *" : "Phone Number *")))}</Label>
                            <Input required type="tel" placeholder="+1 234 567 890" className="h-12 rounded-xl border-slate-200" value={inquiryPhone} onChange={e => setInquiryPhone(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Целевое количество *" : (isFr ? "Quantité cible *" : (isEs ? "Cantidad objetivo *" : (isAr ? "الكمية المطلوبة *" : "Target Quantity *")))}</Label>
                            <Input required placeholder="e.g. 25 MT" className="h-12 rounded-xl border-slate-200" value={inquiryQuantity} onChange={e => setInquiryQuantity(e.target.value)} />
                          </div>
                          <div className="space-y-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Название компании" : (isFr ? "Nom de l'entreprise" : (isEs ? "Nombre de la empresa" : (isAr ? "اسم الشركة" : "Company Name")))}</Label>
                            <Input placeholder="Optional" className="h-12 rounded-xl border-slate-200" value={inquiryCompany} onChange={e => setInquiryCompany(e.target.value)} />
                          </div>
                          <div className="space-y-2 md:col-span-2">
                            <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isRu ? "Дополнительные требования" : (isFr ? "Exigences supplémentaires" : (isEs ? "Requisitos adicionales" : (isAr ? "متطلبات إضافية" : "Additional Requirements")))}</Label>
                            <Textarea 
                              className="min-h-[100px] rounded-xl border-slate-200"
                              placeholder={isRu ? "Опишите ваши потребности..." : (isFr ? "Décrivez vos besoins..." : (isEs ? "Describa sus necesidades..." : (isAr ? "صف احتياجاتك الخاصة..." : "Describe your specific needs...")))}
                          ...
                          <Button type="submit" className="md:col-span-2 h-14 bg-[#0066B3] hover:bg-[#004a82] text-white rounded-xl font-bold text-base mt-2">
                            {isRu ? "Отправить запрос" : (isFr ? "Envoyer la demande" : (isEs ? "Enviar solicitud" : (isAr ? "إرسال الطلب" : "Submit Inquiry")))}
                          </Button>
                        </form>
                      )}
                    </DialogContent>
                    ...
                    <DialogContent className="sm:max-w-[450px] rounded-[2rem] p-8">
                      ...
                      {docSubmitted ? (
                        <div className="py-12 text-center animate-in fade-in zoom-in duration-500">
                          ...
                          <h3 className="text-xl font-bold text-slate-900 mb-2">{isRu ? "Запрос отправлен!" : (isFr ? "Demande envoyée !" : (isEs ? "¡Solicitud enviada!" : (isAr ? "تم إرسال الطلب!" : "Request Sent!")))}</h3>
                          <p className="text-slate-500">{isRu ? "Наша команда свяжется с вами в течение 24 часов." : (isFr ? "Notre équipe vous contactera dans les 24 heures." : (isEs ? "Nuestro equipo se pondrá en contacto con usted en un plazo de 24 horas." : (isAr ? "سيتصل بك فريقنا خلال 24 ساعة." : "Our team will contact you within 24 hours.")))}</p>
                        </div>
                      ) : (
                        <form onSubmit={handleDocSubmit} className="space-y-6 mt-4">
                          <div className="space-y-2">
                            <Label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{isRu ? "Рабочая почта" : (isFr ? "Email professionnel" : (isEs ? "Correo electrónico de trabajo" : (isAr ? "البريد الإلكتروني للعمل" : "Work Email")))}</Label>
                          ...
                          <div className="space-y-2">
                            <Label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{isRu ? "Название компании" : (isFr ? "Nom de l'entreprise" : (isEs ? "Nombre de la empresa" : (isAr ? "اسم الشركة" : "Company Name")))}</Label>
                          ...
                          <div className="flex items-center space-x-2 py-2">
                            <Checkbox 
                              id="doc-subscribe" 
                              checked={docSubscribe} 
                              onCheckedChange={(checked) => setDocSubscribe(checked === true)}
                            />
                            <Label htmlFor="doc-subscribe" className="text-xs text-slate-500 font-medium cursor-pointer">
                              {content.subscribeText}
                            </Label>
                          </div>
                          <Button type="submit" className="w-full h-14 bg-[#0066B3] hover:bg-[#004a82] text-white rounded-xl font-bold text-base">
                            {isRu ? "Отправить запрос" : (isFr ? "Envoyer la demande" : (isEs ? "Enviar solicitud" : (isAr ? "إرسال الطلب" : "Submit Request")))}
                          </Button>
                        </form>
                      )}
                    </DialogContent>
            ...
            <div className="w-full lg:w-1/3 px-8 mb-16 lg:mb-0 order-last">
              <div className="lg:sticky lg:top-32">
                <div className="mb-6 hidden lg:block">
                  <h2 className="text-[11px] font-black text-[#0066B3] uppercase tracking-[0.3em] mb-3">Navigation</h2>
                  <nav className="space-y-1">
                    {[
                      ...
                      <button 
                        ...
                        className="flex items-center w-full p-3 rounded-xl text-left font-bold text-slate-600 hover:bg-white hover:text-[#0066B3] hover:shadow-sm transition-all group"
                      >
                        <span className="w-1.5 h-1.5 rounded-full bg-slate-300 mr-3 group-hover:bg-[#0066B3] transition-colors" />
                        {tab.label}
                      </button>
                    ))}
                  </nav>
                ...
                <div className="mb-8">
                  ...
                  >
                    ...
                    {isFr ? "Discuter sur WhatsApp" : (isEs ? "Chatear en WhatsApp" : (isAr ? "الدردشة على واتساب" : "Chat on WhatsApp"))}
                  </a>
                </div>

                {/* Related Topic Hub Link */}
                <div className="p-8 rounded-[2rem] bg-slate-900 border border-slate-800 shadow-xl mb-8 group hover:-translate-y-1 transition-all duration-500">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-blue-500/20 flex items-center justify-center">
                      <BookOpen className="w-5 h-5 text-blue-400" />
                    </div>
                    <h3 className="text-lg font-black text-white">{content.exploreRelatedTopic}</h3>
                  </div>
                  <p className="text-sm text-slate-400 font-medium mb-6 leading-relaxed">
                    {content.topicDescription}
                  </p>
                  <Link to={`${langPrefix}/topics/${product.slug}`}>
                    <Button className="w-full h-12 bg-white hover:bg-blue-50 text-slate-900 font-bold rounded-xl flex items-center justify-center gap-2 transition-all">
                      {content.viewTopicHub}
          ...
          <div className="container mx-auto px-4">
            <div className="flex items-end justify-between mb-16">
              <div>
                ...
                <h3 className="text-4xl font-black text-white tracking-tight">{isRu ? "Похожие продукты" : (isFr ? "Produits Connexes" : (isEs ? "Productos Relacionados" : (isAr ? "منتجات ذات صلة" : "Related Products")))}</h3>
              </div>
