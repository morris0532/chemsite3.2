...
const LanguageGuide: React.FC = () => {
  ...
  const [targetLangCode, setTargetLangCode] = useState<'en' | 'ru' | 'fr' | 'es' | 'ar'>('en');
  ...
  useEffect(() => {
    ...
    if (matchedLangKey && matchedLangKey !== currentLang) {
      ...
      if (!isDismissed) {
        setTargetLangCode(langMap[matchedLangKey].code as any);
        setTargetLangName(langMap[matchedLangKey].name);
        setShow(true);
      }
    } else {
      setShow(false);
  ...
  // Load content for all potentially needed languages at the top level to follow Hooks rules
  ...
  const getTargetContent = (code: string) => {
    switch (code) {
      case 'en': return enContent;
      case 'ru': return ruContent;
      case 'fr': return frContent;
      case 'es': return esContent;
  ...
  const handleSwitch = () => {
    ...
    const targetLocale = targetLangCode;
    const targetContent = getTargetContent(targetLocale);
  ...
  const handleDismiss = () => {
    sessionStorage.setItem(`dismiss-lang-guide-${targetLangCode}`, 'true');
    setShow(false);
  };
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-900">
            {targetLangCode === 'ar' ? 'هل تفضل تصفح الموقع باللغة العربية؟' :
              targetLangCode === 'ru' ? 'Предпочитаете просматривать сайт на русском языке?' :
                targetLangCode === 'fr' ? 'Préférez-vous naviguer sur le site en français ?' :
                  targetLangCode === 'es' ? '¿Prefieres navegar por el sitio en español?' :
                    `Switch to ${targetLangName}?`}
          </p>
          <div className="mt-3 flex gap-3">
            ...
            >
              {targetLangCode === 'ar' ? 'نعم، قم بالتبديل' :
                targetLangCode === 'ru' ? 'Да, переключить' :
                  targetLangCode === 'fr' ? 'Oui, changer' :
                    targetLangCode === 'es' ? 'Sí, cambiar' :
                      'Yes, switch'}
            </button>
            <button
            ...
            >
              {targetLangCode === 'ar' ? 'ليس الآن' :
                targetLangCode === 'ru' ? 'Не сейчас' :
                  targetLangCode === 'fr' ? 'Pas maintenant' :
                    targetLangCode === 'es' ? 'Ahora no' :
                      'Not now'}
            </button>
          </div>
        </div>
