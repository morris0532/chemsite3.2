...
function safeUrl(url) {
  // Encode spaces and other special characters in URL, but keep slashes and protocol
  const parts = url.split('://');
  if (parts.length < 2) return escapeXml(url.replace(/ /g, '%20'));
  
  const protocol = parts[0];
  const rest = parts[1];
  const encodedRest = rest.split('/').map(segment => encodeURIComponent(segment)).join('/');
  
  // After URI encoding, we still need to escape for XML (especially for &)
  return escapeXml(`${protocol}://${encodedRest}`);
}
...
function formatUrlEntry(loc, changefreq, priority, alternates) {
  ...
  Object.entries(alternates).forEach(([lang, href]) => {
    entry += `
    <xhtml:link rel="alternate" hreflang="${lang}" href="${safeUrl(href)}" />`;
  });
  // Add x-default
  entry += `
    <xhtml:link rel="alternate" hreflang="x-default" href="${safeUrl(alternates['en'] || loc)}" />`;
