...
const contentMetadata = {
  en: { products: [], blog: [], topics: [] },
  ru: { products: [], blog: [], topics: [] },
  fr: { products: [], blog: [], topics: [] },
  es: { products: [], blog: [], topics: [] },
  ar: { products: [], blog: [], topics: [] }
};

['en', 'ru', 'fr', 'es', 'ar'].forEach(locale => {
  ['products', 'blog', 'topics'].forEach(type => {
...
routes.forEach(route => {
  ...
  if (parts.length === 3 && (type === 'blog' || type === 'products' || type === 'topics')) {
    // Find the markdown file by slug in frontmatter or filename
    const contentType = type === 'topics' ? 'products' : type;
    const dir = path.join(contentDir, locale, contentType);
    ...
    const page = parts[1];
    if (page === 'products' || page === 'topics') {
      title = page === 'products' ? `Industrial Chemical Products | Oxalic Acid, Caustic Soda & More` : `Chemical Industry Topics & Applications | Sinopeakchem`;
      description = page === 'products' 
        ? `Browse 22+ high-purity industrial chemicals from Sinopeakchem. Including organic acids, alkali products, sulfates, and specialty chemicals. COA/MSDS available.`
        : `Explore comprehensive chemical industry topics, applications, and technical insights from Sinopeakchem.`;
